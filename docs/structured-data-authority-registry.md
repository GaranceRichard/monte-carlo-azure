...
| SD-07 | Événements, fenêtre, calendrier et périodes delivery | `frontend/src/domain/delivery/` : `DeliveryEvent`, `DeliveryHistoryWindow`, `DeliveryWeek`, `DeliveryHistoryPeriods`, politiques et constructeurs | mapper `adapters/azure-devops/deliveryEventMappers.ts`, fenêtre demandée construite par `getTeamDeliveryDataDirect`, lignes hebdomadaires corrélées | sélection delivery, qualification des bords, throughput, Cycle Time et agrégation corrélée portefeuille | DTO → faits UTC immuables ; bornes absolues → périodes initiale/finale partielles ou cœur complet avec diagnostics ; instant ou lundi ISO → semaine ISO `UTC` identifiée par son lundi | aucune définition concurrente observée |
