...
respectivement la complétude de l’historique requis et sa continuité. Le PBI 7.30 conserve ces résultats dans
le contrat applicatif `TeamHistoryResult` le 9 septembre 2026. Ce registre prolonge les cartographies factuelles
...
| SD-07 | Événements, fenêtre, calendrier, périodes, complétude, continuité, chronologie et Cycle Time delivery | `frontend/src/domain/delivery/` : `DeliveryEvent`, `DeliveryHistory`, `DeliveryHistoryResult`, `DeliveryHistoryWindow`, `DeliveryWeek`, `DeliveryHistoryPeriods`, `DeliveryChronologyResult`, `CycleTimePoint`, politiques et transformations | mapper `adapters/azure-devops/deliveryEventMappers.ts`, attentes WIQL et histoires indisponibles fournies par `getTeamDeliveryDataDirect`, fenêtre demandée et lignes hebdomadaires corrélées | `application/team-history:TeamHistoryResult`, qualifications de complétude, continuité, bords et chronologie, garde de prévision, sélection delivery, throughput, Cycle Time, historique, tendances et agrégation corrélée portefeuille | DTO → faits UTC immuables ; période + attentes + faits → statut complet/incomplet/absent ; attentes + faits + indisponibilités → statut continu/discontinu/ambigu et diagnostics de rupture ; bornes absolues → périodes partielles/cœur complet ; premiers faits ordonnés début/fin/livraison → événements cohérents ou rejetés et diagnostics localisés ; les quatre diagnostics → familles applicatives distinctes par référence ; instant ou lundi ISO → semaine ISO `UTC` ; premiers faits début/fin → jours calendaires à deux décimales | aucune définition concurrente observée |
...
séparément disponibilité/complétude et continuité/ruptures ; `TeamHistoryResult` leur ajoute les diagnostics de
période et de chronologie sous quatre familles qui gardent les références du domaine. La prévision consomme ce
contrat sans seconde détection, puis les calculs utilisent la cohérence déjà fournie. La forme hebdomadaire reste dérivée par l’unique
...
exécutables des autres chemins. La formulation et le signalement visuel de qualité prévus par les Features
ultérieures ne sont pas anticipés.
