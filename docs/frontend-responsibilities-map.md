...
Le PBI 7.27 confie au même domaine la disponibilité et la complétude de l’historique requis. Le client Azure
DevOps conserve le diagnostic `complete`, `incomplete` ou `absent`, throughput et Cycle Time consomment les
...
Contrôles React
  ...
  -> signature de la configuration
     ...
     -> sinon : API publique TeamForecast -> localTeamForecast.runSimulationForecast
        ...
        -> domain/delivery : weeklyThroughput
        -> cycleTimeDaysData + avertissement
...
| Complétude historique | `domain/delivery/historyCompleteness.ts` | période complète disponible + identifiants requis + événements normalisés → résultat delivery et diagnostic `complete`, `incomplete` ou `absent` ; une période disponible sans livraison reste complète. |
