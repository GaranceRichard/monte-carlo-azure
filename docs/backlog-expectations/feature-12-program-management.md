# Feature 12 — Soutenir le pilotage de programme avec des relations opérationnelles explicites

Le résultat observable, le flux de valeur et le statut de la Feature sont définis dans le
[`registre du backlog`](../backlog.md). Ce document ne porte que ses attendus détaillés.

## État du raffinement

La Feature est identifiée et ses PBI sont enregistrés dans le backlog, mais aucun attendu détaillé supplémentaire n’est encore validé.

Avant engagement d’un PBI, préciser au minimum :

- le résultat attendu ;
- le périmètre ;
- le hors périmètre ;
- les dépendances d’entrée ;
- les preuves nécessaires.
