...
def test_repository_backlog_status_and_generated_sections_are_exact() -> None:
    ...
    assert sum(feature.completed_count for feature in features) == 37
    ...
    assert feature_seven.completed_count == 5
    assert [
    ...
    assert "5/75 PBI réalisés (6,67 %)" in backlog
    assert "Prochain PBI :** 7.6 — Chaque donnée structurante" in backlog
    assert "Progression globale :** 37/202 PBI réalisés (18,32 %) ; 165 restants" in backlog
    assert "Répartition actuelle des 165 PBI non réalisés" in governance
    assert (
