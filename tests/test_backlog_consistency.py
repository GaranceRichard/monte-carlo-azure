...
def test_repository_backlog_status_and_generated_sections_are_exact() -> None:
    ...
    assert sum(feature.completed_count for feature in features) == 53
    ...
    assert feature_seven.completed_count == 21
    assert [
    ...
    assert "21/75 PBI réalisés (28 %)" in backlog
    assert (
        "Prochain PBI :** 7.14 — "
        "Les adaptateurs restent indépendants entre eux"
        in backlog
    )
    assert "Progression globale :** 53/204 PBI réalisés (25,98 %) ; 151 restants" in backlog
    assert "Répartition actuelle des 151 PBI non réalisés" in governance
    assert (
