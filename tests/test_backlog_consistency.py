...
def test_repository_backlog_status_and_generated_sections_are_exact() -> None:
    ...
    assert sum(feature.completed_count for feature in features) == 57
    ...
    assert feature_seven.completed_count == 25
    assert [
    ...
    assert "25/75 PBI réalisés (33,33 %)" in backlog
    assert (
        "Prochain PBI :** 7.14 — "
        "Les adaptateurs restent indépendants entre eux"
        in backlog
    )
    assert "Progression globale :** 57/204 PBI réalisés (27,94 %) ; 147 restants" in backlog
    assert "Répartition actuelle des 147 PBI non réalisés" in governance
    assert (
