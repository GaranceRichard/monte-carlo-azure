...
if (-not $ReuseExistingReport) {
    & "$WorkspaceRoot\.venv\Scripts\python.exe" "$WorkspaceRoot\Scripts\report_vitals_coverage.py" --output $vitalsJson
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}
if (-not (Test-Path $vitalsJson)) { throw "Missing Vitals coverage report artifact: $vitalsJson" }

& "$WorkspaceRoot\.venv\Scripts\python.exe" "$WorkspaceRoot\Scripts\check_vitals_compliance.py" --report-json $vitalsJson
