...
test("simulation: erreur puis succes sur les 2 modes", async ({ page }) => {
  ...
  await periodSection.getByRole("button", { name: "Developper" }).click();
  ...
  await filtersSection.getByRole("button", { name: "Developper" }).click();
  ...
  await page.getByLabel("Bug").check();
  await page.getByLabel("Done").check();
  await modeSection.getByRole("button", { name: "Developper" }).click();
